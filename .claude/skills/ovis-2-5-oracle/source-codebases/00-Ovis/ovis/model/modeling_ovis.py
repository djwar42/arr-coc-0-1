"""Ovis Model - Structural Embedding Alignment for Multimodal Large Language Models

Core implementation of the Ovis architecture, featuring Visual Embedding Table (VET)
for structural alignment between visual and textual modalities.
"""

# <claudes_code_comments>
# ** Function List **
# VisualEmbedding.forward - probabilistic embedding lookup via matmul
# VisualEmbedding.reset_parameters - initialize VET weights with normal distribution
# VisualTokenizer.__init__ - initialize ViT, image processor, and visual head
# VisualTokenizer._get_last_block - access final ViT layer for monitoring
# VisualTokenizer._encode - encode visual features through ViT
# VisualTokenizer.smart_resize - intelligent image resizing preserving aspect ratio
# VisualTokenizer.preprocess - convert PIL images to patches with grid encoding
# VisualTokenizer.get_dummy_visual_inputs - create dummy inputs for initialization
# VisualTokenizer.forward - complete visual tokenization pipeline (encode → head → softmax)
# VisualTokenizer.get_monitor_tensors - extract tensors for training monitoring
# Ovis.__init__ - initialize complete multimodal model (VT, VET, LLM)
# Ovis.tie_weights - tie LLM embedding weights
# Ovis.re_init_vte - reinitialize Visual Embedding Table with custom distribution
# Ovis.get_vte - retrieve Visual Embedding Table
# Ovis.get_wte - retrieve text embedding table from LLM
# Ovis.get_conversation_formatter - create conversation formatter for chat templates
# Ovis.forward - training forward pass with loss computation
# Ovis.merge_multimodal - merge visual and text embeddings into unified sequence
# Ovis._merge_inputs - internal merging helper for multimodal inputs
# Ovis.preprocess_inputs - convert messages to model inputs (tokenization + visual preprocessing)
# Ovis.save_pretrained - save model with custom VET/tokenizer handling
# Ovis.generate - inference generation with thinking mode support
# Ovis.chat - high-level chat interface with conversation management
#
# ** Technical Review **
# This module implements the core Ovis architecture with three key components:
# 1. VisualTokenizer: Processes images through ViT → visual head → probability distributions
# 2. VisualEmbedding (VET): Learnable embedding table for discrete visual vocabulary
# 3. Ovis: Main model orchestrating VT, VET, and LLM (Qwen3)
#
# Critical architectural insight: VisualEmbedding performs probabilistic lookup via matmul
# rather than discrete indexing. Given probability distribution p over vocab, output is
# weighted sum: embedding = Σᵢ (pᵢ × embedding_table[i]). This creates structural alignment
# between continuous visual features and discrete text tokens.
#
# VisualTokenizer handles native resolution processing through smart_resize(), which:
# - Ensures dimensions divisible by factor (patch_size × hidden_stride = 28)
# - Constrains total pixels to [min_pixels, max_pixels] range (448² to 1792²)
# - Preserves aspect ratio while meeting constraints
# - Handles edge cases (very small images, extreme aspect ratios)
#
# The preprocess() method creates spatial-temporal patches for ViT input:
# - Reshapes images into grid: (grid_t, temporal_patch_size, channels, grid_h, grid_w, patch_size)
# - Transposes to group hidden_stride patches together
# - Flattens to sequence of patch embeddings
# - Returns (pixel_values, grid_thws) where grid_thws encodes temporal-height-width structure
#
# Ovis.merge_multimodal() is the key integration point:
# - Takes text input_ids and visual embeddings
# - Replaces <image>/<video> token positions with corresponding visual embeddings
# - Creates unified sequence for LLM processing: [text, VISUAL, text, ...]
# - Handles attention masks and labels for training (masks user text, keeps assistant text)
#
# Training flow: forward() → encode visuals → lookup VET → merge → LLM → compute loss
# Inference flow: generate() → same path but with autoregressive generation
# Chat interface: chat() → format conversation → preprocess → generate → decode
#
# Thinking mode (enable_thinking=True) uses two-phase generation:
# Phase 1: Generate reasoning with <think> tags up to thinking_budget tokens
# Phase 2: Generate final answer with remaining tokens
# This enables reflection and self-correction for complex reasoning tasks.
#
# DeepSpeed ZeRO-3 support throughout with GatheredParameters for weight initialization
# and parameter access. Model supports gradient checkpointing and Flash Attention 2.
# </claudes_code_comments>
