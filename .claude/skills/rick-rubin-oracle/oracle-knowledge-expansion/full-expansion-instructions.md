...
### Self-Organization (Part of Oracle Knowledge Expansion)

**Reorganization and maintenance capabilities within Oracle Knowledge Expansion.**

#### Two Paths Within Oracle Knowledge Expansion

**_ingest/** - Manual ingestion (USER drops files)
- User places files manually
- Oracle processes user's content
- Workflow: `_ingest/` → `_ingest/inprocess/` → `_ingest/completed/`

**_ingest-auto/** - Autonomous expansion (ORACLE launches oracle-knowledge-runners)
- Oracle acquires knowledge from web/sources
- Oracle-knowledge-runners execute in parallel
- Web research using Bright Data
- Workflow: `_ingest-auto/` → `_ingest-auto/inprocess/` → `_ingest-auto/completed/`

**Reorganization (direct file editing, no _ingest-auto/)**
- Split/merge/move files directly
