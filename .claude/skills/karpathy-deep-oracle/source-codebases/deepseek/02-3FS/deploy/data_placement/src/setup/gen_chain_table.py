...
# ** Function List **
# calc_target_id(target_id_prefix, node_id, disk_index, target_index) - Generate hierarchical target ID
# generate_chains(chain_table_type, node_id_begin, node_id_end, ...) - Convert incidence matrix to chain assignments
# main() - CLI entry point for chain table generation
#
# ** Technical Review **
# 3FS chain table generator that converts BIBD incidence matrix solutions from data_placement_job.py
# into concrete storage target and chain configurations for cluster deployment.
#
# Data Structures:
# - Target: namedtuple(target_id, node_id, disk_index)
#   * Represents single storage target on specific disk
# - Chain: namedtuple(chain_id, target_list)
#   * Represents replication/EC group with ordered targets
#
# ID Encoding Scheme:
# calc_target_id() - Hierarchical 10-digit encoding:
# - Format: PPNNNNNNDDII
#   * PP (2 digits): target_id_prefix (cluster namespace)
#   * NNNNNN (6 digits): node_id (0-999,999)
#   * DD (2 digits): disk_index+1 (1-999)
#   * II (2 digits): target_index+1 (1-99)
# - Example: 1002003405 = prefix 10, node 20, disk 33, target 4
# - Max constraints:
#   * node_id < 1,000,000
#   * disk_index < 999
#   * target_index < 99
#   * target_id_prefix < 100
#
# chain_id Encoding - Different for EC vs CR:
# - Format: PPDDDCCCCC
#   * PP (2 digits): chain_id_prefix
#   * DDD (3 digits): disk_index+1
#   * CCCCC (5 digits): chain_index (algorithm-specific)
# - CR (Chain Replication): chain_index = group number from incidence matrix
# - EC (Erasure Coding): chain_index = (group-1) * group_size + slot_in_group
#   * Each group spawns group_size chains (one per target)
#   * Slot tracking via group_slot_idx counter
#
# generate_chains() Algorithm:
# 1. Validation:
#    - Verify incidence_matrix size matches num_nodes * num_targets_per_disk
#    - Check all groups have same size
#    - Assert max(nodes) == num_nodes
#
# 2. Triple-nested iteration:
#    - Outer: for disk_index in range(num_disks_per_node)
#    - Middle: for node_id in range(node_id_begin, node_id_end+1)
#    - Inner: for target_index in range(num_targets_per_disk)
#
# 3. Per-target processing:
#    a. Calculate target_id via hierarchical encoding
#    b. Lookup group assignment from incidence_matrix
#       - target_pos = (node_id - node_id_begin) * num_targets_per_disk + target_index
#       - group = groups[target_pos]
#    c. Compute chain_id:
#       * EC: Allocate new chain per target (group_slot_idx tracking)
#       * CR: Map all targets in group to same chain_id
#    d. Create Target object and append to chain_target_list[chain_id]
#
# 4. Post-processing validation:
#    - EC: All chains have exactly 1 target (each target = separate EC stripe)
#    - CR: All chains have group_size targets (chain replication group)
#    - Verify target distribution balance across nodes and disks
#
# Chain Table Types:
# - "EC" (Erasure Coding):
#   * num_chains = num_nodes * num_disks_per_node * num_targets_per_disk
#   * Each chain has 1 target (client assembles EC stripes across chains)
#   * Group size indicates EC stripe width (k in k+m EC)
# - "CR" (Chain Replication):
#   * num_chains = num_nodes * num_disks_per_node * num_targets_per_disk / group_size
#   * Each chain has group_size targets (CRAQ replication chain)
#   * Targets ordered for chain replication protocol
#
# Output Files:
# 1. generated_chains.csv:
#    - Format: ChainId,TargetId,TargetId,...
#    - Full chain→target mapping for validation
#
# 2. generated_chain_table.csv:
#    - Format: ChainId (single column)
#    - Minimal chain registry for metadata service
#
# 3. create_target_cmd.txt:
#    - 3FS CLI commands for target creation
#    - Format: create-target --node-id N --disk-index D --target-id T --chain-id C [--chunk-size ...]
#    - --use-new-chunk-engine flag enables optimized chunk engine
#
# 4. remove_target_cmd.txt:
#    - 3FS CLI commands for cleanup
#    - Two-step: offline-target (graceful drain) → remove-target (delete)
#
# CLI Arguments:
# - --chain_table_type: "EC" or "CR"
# - --node_id_begin/end: Node ID range (e.g., 0-199 for 200 nodes)
# - --num_disks_per_node: Disks per storage node (e.g., 16 NVMe SSDs)
# - --num_targets_per_disk: Targets per disk (replication factor from BIBD)
# - --target_id_prefix: Namespace prefix for target IDs (default: 10)
# - --chain_id_prefix: Namespace prefix for chain IDs (default: 10)
# - --chunk_size: List of supported chunk sizes (e.g., "4M 64M 1G")
# - --incidence_matrix_path: Pickled dict from data_placement_job.py
# - --output_path: Directory for generated files
#
# Integration Pipeline:
# ```
# data_placement_job.py (BIBD solver)
#   → incidence_matrix.pkl
#     → gen_chain_table.py (this script)
#       → generated_chains.csv
#       → generated_chain_table.csv
#       → create_target_cmd.txt
#         → Execute on 3FS cluster (create targets)
# ```
#
# Example:
# ```bash
# python gen_chain_table.py \
#   --chain_table_type EC \
#   --node_id_begin 0 \
#   --node_id_end 199 \
#   --num_disks_per_node 16 \
#   --num_targets_per_disk 10 \
#   --incidence_matrix_path solution.pkl \
#   --chunk_size 4M 64M 1G \
#   --output_path ./configs
# ```
#
# Constraints Enforcement:
# - incidence_matrix size < 100,000 (chain_index 5-digit limit)
# - Node range < 1000 nodes (NNNNNN 6-digit encoding supports up to 1M)
# - num_disks_per_node < 1000
# - num_targets_per_disk < 100
# - Prefixes < 100 (2-digit encoding)
# </claudes_code_comments>
