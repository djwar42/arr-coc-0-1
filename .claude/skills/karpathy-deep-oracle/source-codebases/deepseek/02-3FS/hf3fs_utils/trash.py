...
# ** Function List **
# format_date(t) - Format datetime to YYYYmmdd_HHMM in UTC+8
# parse_date(t) - Parse YYYYmmdd_HHMM string to datetime in UTC+8
# get_timestamp_us() - Get current timestamp in microseconds
# TrashConfig.__post_init__() - Validate trash configuration parameters
# TrashConfig.current_dir() - Generate time-sliced trash directory name
# Trash.__init__(filesystem, user, user_name) - Initialize user trash handler
# Trash._check_user() - Verify effective user matches trash owner
# Trash.move_to_trash(path, config, trash_name, append_timestamp_if_exists) - Move file/dir to trash with expiration
#
# ** Technical Review **
# Time-based trash management system for 3FS with automatic cleanup scheduling.
#
# Architecture:
# - Uses UTC+8 timezone (UTC8_TZ) for all timestamps
# - BASE_TIMESTAMP (1980-01-01) as epoch for time slice calculations
# - Microsecond precision timestamps for collision avoidance
#
# TrashConfig system:
# - Defines expiration policy (expire) and time slice granularity (time_slice)
# - Six predefined configs: 1h, 3h, 8h, 1d, 3d, 7d
# - current_dir() generates directory names: "{name}-{start}-{end}" format
# - Time slices partition the timeline for batch cleanup operations
# - Example: "1h-20251028_1400-20251028_1510" (1 hour expire, 10 min slices)
#
# Trash class workflow:
# 1. Validates user trash directory exists: /{mountpoint}/trash/{username}
# 2. Verifies ownership (st_uid must match user)
# 3. move_to_trash() creates time-sliced subdirectory if needed
# 4. Uses FileSystem._rename_ioctl() with move_to_trash=True flag
# 5. Handles name collisions: appends microsecond timestamp (up to 10 retries)
# 6. Truncates names to 200 chars before timestamp to stay under 255-byte limit
#
# Safety features:
# - Root user blocked
# - Directory owner + rwx permission checks (0o700)
# - Atomic operations via ioctl
# - Collision detection with automatic timestamp suffixing
#
# Time slice strategy enables efficient bulk deletion: cleanup processes scan
# directory names, parse expiration times, and delete expired time slices atomically.
# </claudes_code_comments>
