...
# ** Function List **
# get_filesystem(path) - Detect and validate 3FS mount point from path
# abs_path(path) - Normalize path while preserving symlinks in filename
# abort(msg) - Print error message and exit with status 1
# cli() - Main Click command group entry point
# mv(old_path, new_path) - Move files between 3FS mount points
# ExpireType - Custom Click parameter type for trash expiration times
# rmtree(dir_paths, expire, yes) - Move directories to trash with expiration
#
# ** Technical Review **
# 3FS CLI utility providing safe file operations on Fire-Flyer File System. Core flow:
# get_filesystem() → validates mount point via 3fs-virt directory → FileSystem object.
# Main commands: mv() for cross-mount moves, rmtree() for trash-based deletion.
# Trash system: Time-based expiration (1h|3h|8h|1d|3d|7d) with automatic cleanup.
# Safety features: Prevents '..' traversal, root user blocking, symlink handling.
# Path resolution: Preserves symlinks in filenames while resolving directory paths.
# Uses ioctl-based operations for atomic 3FS operations (rename, remove).
# Expiration configs in TRASH_CONFIGS dict map to TrashConfig objects with time slices.
# </claudes_code_comments>
