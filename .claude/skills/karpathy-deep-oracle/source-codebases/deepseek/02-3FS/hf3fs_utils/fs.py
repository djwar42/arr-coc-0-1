...
# ** Function List **
# is_relative_to(path1, path2) - Check if path1 is relative to path2
# FileSystem.__init__(mountpoint) - Initialize and validate 3FS mount point
# FileSystem._check_user() - Prevent root user operations
# FileSystem._encode_filename(name) - Encode filename to UTF-8 bytes with 255 char limit
# FileSystem.opendir(dir_path) - Open and validate directory on 3FS
# FileSystem.split_path(path) - Split path into dir_fd, dir_stat, and filename
# FileSystem.rename(old_path, new_path) - Atomic rename operation via ioctl
# FileSystem._rename_ioctl() - Low-level ioctl for rename with trash support
# FileSystem.remove(path, recursive) - Remove file or directory via ioctl
# FileSystem._remove_ioctl() - Low-level ioctl for deletion
# FileSystem._ioctl(fd, cmd, buffer) - Execute ioctl system call with user check
#
# ** Technical Review **
# Core abstraction layer for 3FS operations using ioctl system calls. Architecture:
# - Validates mount points via "3fs-virt" directory and magic number (0x8F3F5FFF)
# - Checks ioctl version support (minimum version 1 required)
# - Blocks root user operations for safety (_check_user)
# - All operations use file descriptor + inode number pairs for atomicity
#
# Key ioctl commands:
# - HF3FS_IOCTL_MAGIC_CMD (0x80046802): Validate 3FS mount
# - HF3FS_IOCTL_VERSION_CMD (0x80046803): Check ioctl API version
# - HF3FS_IOCTL_RENAME_CMD (0x4218680E): Atomic rename with 536-byte buffer
# - HF3FS_IOCTL_REMOVE_CMD (0x4110680F): Atomic remove with 272-byte buffer
#
# Safety features:
# - Virtual path detection (high-order bits in inode: 0xF000000000000000)
# - Symlink protection (operations fail on symlinks)
# - Permission validation (owner + rwx for directory operations)
# - Cross-device checks (st_dev validation)
# - UTF-8 filename encoding with 255-byte limit (ENAMETOOLONG)
#
# Buffer formats use struct.pack with 'N' (native size_t) for inode numbers.
# </claudes_code_comments>
