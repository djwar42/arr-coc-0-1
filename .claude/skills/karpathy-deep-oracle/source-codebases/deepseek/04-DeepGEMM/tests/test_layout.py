...
# ** Function List **
# get_mn_major_tma_aligned_packed_ue8m0_tensor_torch_impl(x) - PyTorch reference for UE8M0 packing
# test_sf_layout_kernels() - Validate scaling factor layout transformations (transpose + UE8M0)
# test_k_grouped_sf_layout_kernels() - Validate K-grouped scaling factor UE8M0 packing
#
# ** Technical Review **
# Scaling factor (SF) layout transformation tests for SM90/SM100 compatibility.
#
# Background: FP8 GEMMs require per-block scaling factors. SM90 uses FP32 scales in K-major layout,
# SM100 requires MN-major layout with UE8M0 packing (4 FP32 → 1 torch.int of packed exponents).
# These kernels transform SF layouts for hardware requirements.
#
# get_mn_major_tma_aligned_packed_ue8m0_tensor_torch_impl(): Pure PyTorch reference implementation.
# - Extract exponents: (x.view(torch.int) >> 23).to(torch.uint8) isolates IEEE-754 exponent bits
# - TMA alignment: pad MN to get_tma_aligned_size(mn, 4) for SM100 TMA memory transfers
# - Pack 4-to-1: 4 uint8 exponents → 1 torch.int via view dtype reinterpretation
# - Transpose: K-major → MN-major via .mT (strided transpose without copy)
#
# test_sf_layout_kernels(): Tests both transpose (FP32) and UE8M0 packing modes.
# Correctness checks:
# - UE8M0 mode: Validates packed values and shapes/strides match reference impl
# - FP32 mode: Validates transpose produces MN-major strided layout (stride=(1, tma_aligned_mn))
# - Grouped support: num_groups dimension preserved (for batched MoE)
#
# Performance: Measures GB/s for memory-bound layout transformations. Some cases fallback to
# PyTorch impl (t=0) if specialized kernel unavailable.
#
# test_k_grouped_sf_layout_kernels(): K-grouped variant for MoE weight backward.
# - Variable K per expert: ks list with different K dimensions
# - Split and pack independently: Each expert's SF chunk gets UE8M0 packed separately
# - Validation: Splits output by packed_sf_ks, compares each chunk against torch impl
#
# TMA alignment rationale: SM100 Tensor Memory Accelerator requires specific alignment
# (typically 128 bytes) for async memory transfers. Padding ensures hardware compatibility.
#
# UE8M0 format: Unsigned 8-bit exponent-only (no sign/mantissa). Represents power-of-2 scales
# efficiently. 4 exponents packed into 32-bit int reduces memory by 4x vs FP32.
# </claudes_code_comments>
