"""DualPipeV - V-Shape Pipeline Parallelism (Cut-in-Half DualPipe)"""

# <claudes_code_comments>
# ** Function List **
# __init__(modules, batch_dim, process_group, rank_mapping) - Initialize V-shape pipeline parallelism
...
#
# ** Technical Review **
# DualPipeV is a V-shaped schedule variant of DualPipe derived using "cut-in-half" procedure.
# Introduced by Sea AI Lab, it achieves same bubble reduction as DualPipe with HALF the devices.
#
# Key Difference from DualPipe: Single unidirectional V-shape flow instead of bidirectional.
# Data flows forward through pipeline, then loops back in reverse at the last rank.
# Only first rank provides inputs and receives final outputs/loss (simpler I/O than DualPipe).
#
# Architecture: Each device still holds TWO pipeline stages (modules[0] and modules[1]).
# Phase 0: Forward direction (rank 0 → rank N-1)
# Phase 1: Reverse direction (rank N-1 → rank 0)
# At last rank: phase 0 output becomes phase 1 input (creating V-shape)
#
# Pipeline Schedule (identical 8 steps to DualPipe):
# 1. nF0: Warmup forward chunks in phase 0
# 2. nF0F1: Interleaved forward chunks (creates V-shape at last rank)
# 3. nB1W1F1: Backward-weight-forward overlap with zero-bubble
# 4. nF0B1F1B0: Main steady state with full overlap
# 5. nB1F1B0: Continue overlapped forward-backward
# 6. nB1B0: Cooldown backward chunks
# 7. nWB0: Weight-backward overlap with zero-bubble
# 8. nW: Final weight gradient updates
#
# Performance Characteristics:
# - Bubble: (PP/2-1)(F&B+B-3W) - Same as DualPipe
# - Parameters per device: 2x - Same as DualPipe
# - Activation per device: PP+1 - Same as DualPipe
# - Number of devices: PP/2 - HALF of DualPipe! This is the key advantage
#
# V-Shape Mechanism: At last rank, forward outputs (phase 0) are detached and fed as inputs
# to reverse direction (phase 1). Gradients flow back through both stages on same device.
# At last rank specifically: phase 1 input_grads become phase 0 output_grads (closing the V).
#
# Trade-offs: More complex data routing at boundaries but significantly fewer devices needed.
# Simpler for end users - only first rank needs to provide inputs/labels and receive outputs.
#
# Credit: "Cut-in-half" optimization introduced by Sea AI Lab in their blog post.
# </claudes_code_comments>
