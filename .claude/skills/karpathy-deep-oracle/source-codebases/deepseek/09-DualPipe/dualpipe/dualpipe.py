...
# ** Function List **
# __init__(modules, batch_dim, process_group, rank_mapping) - Initialize bidirectional pipeline parallelism
# _reset_states() - Clear all state for new training step
# _forward_compute_chunk(phase) - Execute forward pass for one micro-batch chunk
# _backward_compute_chunk(phase, enable_zb) - Execute backward pass for one micro-batch chunk
# _forward_backward_compute_chunk(phase0, phase1) - Overlap forward and backward computation
# _forward_chunk(phase, recv, send) - Forward pass with communication
# _backward_chunk(phase, enable_zb, recv, send) - Backward pass with communication
# _forward_backward_chunk(phase0, phase1, recv0) - Overlapped forward-backward with communication
# _weight_chunk() - Process weight gradient updates using zero-bubble optimization
# _free_tensors() - Release tensor memory after communication
# _recv_forward(phase) - Receive forward activations from neighboring rank
# _send_forward(phase) - Send forward activations to neighboring rank
# _recv_backward(phase) - Receive backward gradients from neighboring rank
# _send_backward(phase) - Send backward gradients to neighboring rank
# _commit_and_wait_comm() - Execute batched communication operations
# step(inputs, num_chunks, criterion, labels, return_outputs) - Main entry point for training/inference step
#
# ** Technical Review **
# DualPipe implements bidirectional pipeline parallelism introduced in DeepSeek-V3 technical report.
# Key innovation: achieves full overlap of forward and backward computation-communication phases.
#
# Architecture: Each device holds TWO pipeline stages (modules[0] and modules[1]) instead of one.
# Data flows in both forward and reverse directions simultaneously, creating a bidirectional pipeline.
# First half of ranks: phase 0 = forward direction, phase 1 = reverse direction.
# Second half of ranks: phase 0 = reverse direction, phase 1 = forward direction.
# This phase swap (using XOR with is_in_second_half) enables symmetric bidirectional flow.
#
# Pipeline Schedule (8 steps):
# 1. nF0: Warmup forward chunks in phase 0
# 2. nF0F1: Interleaved forward chunks in both directions
# 3. nB1W1F1: Backward-weight-forward overlap with zero-bubble optimization
# 4. nF0B1F1B0: Main steady state with full computation-communication overlap
# 5. nB1F1B0: Continue overlapped forward-backward
# 6. nB1B0: Cooldown backward chunks in both directions
# 7. nWB0: Weight-backward overlap with zero-bubble optimization
# 8. nW: Final weight gradient updates
#
# Zero-Bubble Optimization: WeightGradStore defers weight gradient computation during backward pass,
# allowing it to overlap with subsequent forward/backward chunks. Reduces pipeline bubbles from
# (PP-1)(F+B) in 1F1B to (PP/2-1)(F&B+B-3W) in DualPipe, nearly 2x reduction.
#
# Communication Strategy: Uses PyTorch distributed P2P operations with batch_isend_irecv for
# efficient async communication. Computation-communication overlap achieved by queuing ops and
# committing them together, allowing compute to proceed while transfers execute.
#
# Memory Trade-off: Stores 2x parameters per device (two pipeline stages) but achieves
# significantly lower bubble rate. Activation memory is PP+1 chunks (similar to 1F1B).
#
# Requirements: Even number of PP stages, even number of micro-batches >= num_ranks * 2.
# Custom modules can implement overlapped_forward_backward() method for finer-grained overlap.
# </claudes_code_comments>
