"""Example usage of DualPipe - Bidirectional Pipeline Parallelism
...
3. DualPipe setup with bidirectional data flow
...
# test_dualpipe(ngpus) - Launch distributed training with torch.multiprocessing
#
# ** Technical Review **
# This example demonstrates proper DualPipe usage with zero-bubble optimization.
#
# Key Implementation Pattern - Custom Autograd with Zero-Bubble:
# LinearFunc.backward() shows how to integrate with WeightGradStore. The weight gradient
# computation (expensive matrix multiplication) is wrapped in grad_weight_fn() and queued
# when WeightGradStore.enabled is True. This allows DualPipe to defer weight gradient
# computation, enabling overlap with subsequent forward/backward passes.
#
# Critical: Weight gradients use += (accumulation) not = (assignment). Multiple micro-batches
# accumulate gradients in the same weight tensor to produce correct full-batch gradient.
#
# Overlapped Forward-Backward:
# PipelineStage.overlapped_forward_backward() is optional but recommended. It allows
# module0's forward and module1's backward to execute truly concurrently (if supported
# by hardware/CUDA streams). This example shows a simple sequential implementation, but
# real-world usage could leverage CUDA streams for finer-grained overlap.
#
# DualPipe Data Distribution:
# In bidirectional pipeline, data splits in half:
# - First rank: receives first half of inputs, second half of labels
# - Last rank: receives second half of inputs, first half of labels
# - Middle ranks: receive no direct inputs/labels (data flows through pipeline)
#
# This bidirectional pattern explains why first_rank gets full_x.chunk(2)[0] but
# full_l.chunk(2)[1] - inputs and labels flow in opposite directions!
#
# Model Partitioning:
# Each rank loads TWO pipeline stages (rank i and rank PP-1-i).
# Example with 4 ranks (PP=4): rank 0 gets stages 0 and 3, rank 1 gets stages 1 and 2.
# This symmetric loading enables bidirectional flow.
#
# Gradient Verification:
# After DualPipe step, gradients from two stages must be combined:
# p0.grad += p1.grad from opposite rank (via all-gather).
# This matches reference (non-pipeline) gradients to verify correctness.
#
# The cal_diff < 1e-13 assertion ensures numerical accuracy - pipeline produces
# mathematically identical gradients to non-pipeline reference implementation.
#
# Multi-GPU Testing:
# Main function runs on all available GPU counts (in steps of 2, since DualPipe
# requires even number of ranks). Tests progressively smaller configurations
# to verify correctness at all scales.
# </claudes_code_comments>
