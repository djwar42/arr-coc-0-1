...
# ** Function List **
# simulate_failure_and_skip(rank, api, expected_masked_ranks) - Simulates rank failures for shrink testing
# query_mask_buffer_and_check(api, buffer, mask_status, expected_masked_ranks) - Verifies rank failure detection
# test_main(num_tokens, hidden, num_experts, num_topk, rank, num_ranks, group, buffer, use_logfmt, shrink_test, seed) - Core low-latency testing with correctness checks and bandwidth benchmarks
# large_gemm_with_hook(hook) - Simulates computation to test hook-based overlapping
# test_func(return_recv_hook) - Combined dispatch+combine test for bandwidth measurement
# test_loop(local_rank, num_local_ranks, args) - Main test orchestration with multi-process setup
#
# ** Technical Review **
# This test suite comprehensively validates DeepEP's low-latency mode designed for ultra-low-latency
# MoE inference (critical for DeepSeek-V3 decoding). Low-latency mode uses NVSHMEM+IBGDA for RDMA
# with asymmetric-domain bandwidth forwarding: ~50 GB/s RDMA + ~160 GB/s NVLink simultaneously.
#
# Core testing strategy:
# 1. **Correctness validation**: Tests dispatch/combine with multiple configurations (FP8/BF16,
#    sync/async, with/without hooks, LogFMT compression, round-scale quantization, UE8M0 format).
#    Validates expert routing, token layout, data integrity across all 288 experts with top-k=8.
#
# 2. **Failure simulation** (shrink_test mode): Simulates rank failures at dispatch/combine/clean APIs
#    to test dynamic shrinking where failed ranks are masked and excluded from communication.
#    Uses mask_status tensor to track failures and validates exclusion logic.
#
# 3. **Performance benchmarking**: Measures dispatch+combine bandwidth (targeting ~210 GB/s combined),
#    separate dispatch/combine times, and send/recv hook timings. Tests overlapping via return_recv_hook.
#
# Key innovations tested:
# - **FP8 quantization**: Per-token E4M3 casting with 128-channel scaling factors, round-scale for
#   reduced precision drift, UE8M0 packed format for SM100 (Blackwell).
# - **LogFMT compression**: 10-bit logarithmic format for combine (1.25 bytes/value vs 2 bytes BF16).
# - **Hook-based overlapping**: return_recv_hook=True enables async receive hooks that can be called
#   during computation, overlapping communication with GEMM workloads.
# - **Zero-copy combine**: Preallocated combine buffer accessed via get_next_low_latency_combine_buffer
#   eliminates memcpy overhead for LogFMT mode.
#
# Algorithm details:
# - Expert assignment: Randomly generates scores → top-k selection → routing to 256-288 experts
#   distributed across num_ranks (typically 8 GPUs). Each rank owns num_local_experts.
# - Data verification: Uses rank-specific patterns (rank - rank_offset) and token indices in last
#   128 channels to validate correct expert routing and data integrity.
# - Hash verification (pressure test): XOR hashing of all intermediate tensors ensures bitwise
#   reproducibility across 1 billion random seeds.
#
# Performance characteristics:
# - Dispatch: ~50 GB/s RDMA for inter-node + ~160 GB/s NVLink for intra-node (FP8)
# - Combine: ~50 GB/s RDMA + ~100 GB/s NVLink (LogFMT10 or BF16)
# - Total latency: ~50-100 μs per dispatch+combine (batch_size=128, hidden=7168, top-k=8)
#
# Edge cases handled:
# - Masked experts (topk_idx=-1): Represents experts not selected by gating
# - Empty expert assignments: Some experts may receive zero tokens
# - NVLink connectivity: Validates pairwise NVLink for PCIe A100 (max EP=2)
# - Numerical precision: Different tolerances for FP8 (9e-4) vs BF16 (1e-5)
# </claudes_code_comments>
