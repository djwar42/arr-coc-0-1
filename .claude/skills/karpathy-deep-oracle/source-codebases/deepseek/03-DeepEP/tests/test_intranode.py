...
# ** Function List **
# check_data(check_x, rank_prefix_matrix) - Validates received data has correct rank-specific patterns
# test_main(args, num_sms, local_rank, num_ranks, rank, buffer, group) - Core intranode testing with NVLink-only communication
# test_loop(local_rank, num_local_ranks, args) - Main test orchestration for single-node setup
#
# ** Technical Review **
# This test suite validates DeepEP's normal mode for intranode expert-parallel communication using
# pure NVLink connectivity (no RDMA). Designed for single-node MoE training where all GPUs are
# connected via NVLink, achieving ~160 GB/s bidirectional bandwidth per GPU on H800/A100 systems.
#
# Core testing strategy:
# 1. **Layout computation validation**: Tests get_dispatch_layout() which calculates token routing
#    metadata (num_tokens_per_rank, is_token_in_rank, num_tokens_per_expert) from topk_idx.
#    This preprocessing step is crucial for efficient all-to-all communication planning.
#
# 2. **Dispatch/combine correctness**: Tests all combinations of:
#    - Data formats: BF16 (baseline), FP8 E4M3 (1.5x compression), pure random data
#    - Modes: sync/async finish, with/without previous event chaining, with/without top-k metadata
#    - Special features: num_worst_tokens (allocates extra buffer for worst-case routing),
#      cached dispatch (reuses layout handle for multiple dispatches), zero-copy combine
#
# 3. **Performance tuning**: Grid search over nvl_chunk_size (4-32) and num_sms (24 default) to
#    find optimal NVLink kernel launch configuration for dispatch and combine operations.
#    Reports bandwidth in GB/s for both operations.
#
# Key algorithm details:
# - **Token routing**: Each token can be routed to multiple ranks based on expert selection.
#   rank_idx tensor tracks which ranks each token needs to be sent to. inplace_unique() ensures
#   each rank appears at most once per token (avoiding duplicate sends).
#
# - **Layout calculation**: Computes token_idx_in_rank which maps global token indices to local
#   indices on receiving ranks. This enables efficient gathering/scattering during all-to-all.
#
# - **FP8 optimization**: Uses per_token_cast_to_fp8() which applies E4M3 quantization with
#   per-token 128-channel scaling factors. Transpose-contiguous pattern ensures correct memory
#   layout for NVLink transfers.
#
# - **Combine aggregation**: Sums expert outputs weighted by topk_weights, then divides by
#   is_token_in_rank.sum() to get per-token averages across selected experts.
#
# Performance characteristics:
# - Dispatch bandwidth: ~160 GB/s (BF16), ~210 GB/s (FP8) with optimal nvl_chunk_size
# - Combine bandwidth: ~120-160 GB/s depending on chunk size
# - Layout computation: <1 ms for 4096 tokens, 256 experts
#
# Design choices:
# - nvl_buffer_size=256: Optimized for 8-GPU nodes, balances memory vs latency
# - Config tuning: Separate optimal configs for dispatch (larger chunks) and combine (smaller chunks)
# - Compatibility testing: Can optionally test low-latency mode on same buffer instance
#
# Edge cases:
# - Empty expert assignments: Some experts may receive zero tokens (validated via recv_count)
# - Masked routing: topk_idx=-1 represents non-selected experts (excluded from routing)
# - Random vs structured data: Both tested to catch layout bugs vs numerical issues
# - Previous event chaining: Tests sequential kernel launches with dependency tracking
# </claudes_code_comments>
