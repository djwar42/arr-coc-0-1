...
# ** Function List **
# check_data(check_x, recv_gbl_rank_prefix_sum) - Validates received data patterns across multi-node setup
# test_main(args, num_sms, local_rank, num_local_ranks, num_ranks, num_nodes, rank, buffer, group, skip_benchmark) - Core multi-node testing with RDMA+NVLink hybrid communication
# test_loop(local_rank, num_local_ranks, args) - Main test orchestration for multi-node distributed setup
#
# ** Technical Review **
# This test suite validates DeepEP's internode mode for multi-node expert-parallel communication
# using hybrid RDMA+NVLink. Critical for large-scale MoE training (e.g., DeepSeek-V3 on 2048 H800 GPUs).
# Achieves asymmetric-domain bandwidth forwarding: ~50 GB/s RDMA between nodes + ~160 GB/s NVLink
# within nodes, enabling efficient expert parallelism across 16-256 nodes.
#
# Core testing strategy:
# 1. **Group-limited gating validation**: Tests num_topk_groups which restricts expert selection
#    to top-k groups of experts (each group spans one node's experts). This reduces cross-node
#    communication by limiting how many nodes each token is routed to. Default: min(num_nodes, 4)
#    groups out of num_nodes total.
#
# 2. **Hybrid communication correctness**: Validates RDMA dispatch (inter-node) + NVLink dispatch
#    (intra-node) in both directions. Tests with:
#    - Data formats: BF16 (baseline), FP8 E4M3 (1.5x compression), pure random data
#    - Bias addition: Tests combine with dual bias tensors (bias_0 + bias_1) to validate
#      accumulation logic in multi-source scenarios
#    - Top-k metadata: Routes both activations and gating weights across hybrid network
#
# 3. **Performance tuning**: 2D grid search over (nvl_chunk_size, rdma_chunk_size) to find optimal
#    kernel configurations for both dispatch and combine. Separately reports RDMA and NVLink bandwidth.
#    Uses Kineto profiling to measure send/recv separately (notify time vs transfer time).
#
# 4. **Hash verification (pressure test)**: XOR hashing across all intermediate tensors with multiple
#    random seeds ensures bitwise reproducibility. Critical for catching race conditions in hybrid
#    RDMA+NVLink communication.
#
# Key algorithm innovations:
# - **Group-limited gating**: create_grouped_scores() masks scores outside selected groups with
#   -inf, ensuring topk only selects from allowed expert groups. Reduces cross-node traffic by
#   ~75% (4 groups out of 16 nodes = 25% traffic vs all-to-all).
#
# - **Dual-level routing**:
#   * rdma_idx: Which nodes to send to (inter-node routing)
#   * rank_idx: Which ranks to send to (intra-node routing)
#   Both use inplace_unique() to avoid duplicate sends per token.
#
# - **Hierarchical layout**:
#   * num_tokens_per_rdma_rank: Tokens sent between nodes via RDMA
#   * num_tokens_per_rank: Tokens distributed within nodes via NVLink
#   * num_tokens_per_expert: Final distribution to local experts
#
# - **Bias addition in combine**: Tests accumulation of expert outputs + dual biases, validating
#   numerics when multiple sources (experts, biases) are summed before reduction.
#
# Performance characteristics (24-node, 192 GPUs):
# - Dispatch RDMA: ~40-50 GB/s inter-node (limited by InfiniBand EDR/HDR)
# - Dispatch NVLink: ~160 GB/s intra-node
# - Combine RDMA: ~40-50 GB/s
# - Combine NVLink: ~120 GB/s
# - Notify latency: ~10-20 μs (RDMA signaling overhead)
#
# Design choices:
# - rdma_buffer_size=128, nvl_buffer_size=512-720 (depends on num_ranks)
# - num_qps_per_rank=24: One queue pair per SM for maximum RDMA parallelism
# - Buffer allocation: 2GB NVLink + 1GB RDMA per rank (total ~3GB per GPU)
# - Pressure test modes: 0=single run, 1=pressure without benchmarks, 2=pressure with benchmarks
#
# Edge cases:
# - Cross-node empty assignments: Some nodes may receive zero tokens from certain source nodes
# - Group masking: Experts outside selected groups should never be routed to
# - RDMA failure handling: Compatible with low-latency shrink mode for fault tolerance
# - Numerical precision: FP8 E4M3 has higher error (5e-4) due to cross-node accumulation
# </claudes_code_comments>
