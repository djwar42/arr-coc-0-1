...
# ** Function List **
# Buffer.__init__() - Initialize EP communication buffer with NVLink/RDMA/IBGDA support
# Buffer.destroy() - Explicitly release C++ runtime resources
# Buffer.set_num_sms() - Configure SM count for high-throughput kernels
# Buffer.capture() - Capture CUDA event on current stream
# Buffer.get_low_latency_rdma_size_hint() - Calculate minimum RDMA buffer size
# Buffer.get_comm_stream() - Get dedicated communication stream
# Buffer.get_local_buffer_tensor() - Access raw buffer as PyTorch tensor
# Buffer._unpack_bias() - Unpack bias tensors from single or tuple
# Buffer.get_dispatch_config() - Get auto-tuned dispatch config for EP rank count
# Buffer.get_combine_config() - Get auto-tuned combine config for EP rank count
# Buffer.get_dispatch_layout() - Pre-calculate token routing layout
# Buffer.dispatch() - MoE dispatch (all-to-all scatter) for intranode/internode
# Buffer.combine() - MoE combine (all-to-all gather+reduce) for intranode/internode
# Buffer.internode_dispatch() - Internode dispatch with NVLink+RDMA hybrid
# Buffer.internode_combine() - Internode combine with NVLink+RDMA hybrid
# Buffer.clean_low_latency_buffer() - Zero-initialize buffer for low-latency mode
# Buffer.low_latency_dispatch() - Ultra-low-latency dispatch with IBGDA and FP8
# Buffer.low_latency_combine() - Ultra-low-latency combine with IBGDA and optional LogFMT
# Buffer.low_latency_update_mask_buffer() - Mask/unmask ranks for dynamic topology
# Buffer.low_latency_query_mask_buffer() - Query current mask status
# Buffer.low_latency_clean_mask_buffer() - Reset all masks
# Buffer.get_next_low_latency_combine_buffer() - Get RDMA buffer for zero-copy combine
#
# ** Technical Review **
# DeepEP's Buffer class is the core abstraction for expert-parallel (EP) communication in MoE models,
# implementing asymmetric-domain bandwidth forwarding optimized for DeepSeek-V3's group-limited gating.
# It provides three operational modes: high-throughput intranode (NVLink), high-throughput internode
# (NVLink+RDMA hybrid), and low-latency (pure RDMA with IBGDA).
#
# Architecture: Python Buffer wraps C++ deep_ep_cpp.Buffer → manages IPC handles and NVSHMEM setup →
# provides dispatch/combine primitives → supports both cached (handle-based) and non-cached execution.
# The dispatch operation scatters tokens to experts across ranks, while combine gathers and reduces them.
#
# Key design choices:
# 1. Dual-mode operation: Normal mode for training/prefill (high throughput, CPU sync for recv count),
#    low-latency mode for inference decoding (pure RDMA, CUDA graph compatible, no CPU sync)
# 2. Asymmetric bandwidth forwarding: Internode kernels route NVLink traffic → RDMA traffic separately,
#    maximizing both domains (DeepSeek-V3 achieves ~50 GB/s RDMA + ~160 GB/s NVLink simultaneously)
# 3. Handle caching: Reuse layout computation (rank_prefix_matrix, channel_prefix_matrix) to eliminate
#    redundant CPU work in multi-batch scenarios
# 4. FP8 support: Low-latency dispatch can cast to FP8 (torch.float8_e4m3fn) with per-128-element scales,
#    or use UE8M0 packed format for minimal overhead
# 5. Hook-based overlapping: low_latency_dispatch/combine return hooks that defer RDMA recv completion,
#    enabling double-buffering without SM occupation (network traffic runs in background)
#
# Communication flow (normal mode):
# 1. get_dispatch_layout() → compute token routing (which tokens go to which ranks/experts)
# 2. dispatch() → all-to-all scatter with NVLink or NVLink+RDMA, returns recv_x and handle
# 3. [Expert computation on recv_x]
# 4. combine() → all-to-all gather+reduce with handle, returns original token ordering
#
# Low-latency mode optimizations:
# - Pure RDMA with IBGDA (InfiniBand Global Direct Access) for minimal latency
# - Multi-QP per rank (num_qps_per_rank = num_local_experts) for parallelism
# - Zero-copy option: get_next_low_latency_combine_buffer() returns pre-registered RDMA buffer
# - Preallocated buffers: num_max_dispatch_tokens_per_rank avoids dynamic sizing (CUDA graph safe)
# - Optional NVLink traffic for intranode low-latency (allow_nvlink_for_low_latency_mode)
#
# Performance tuning via Config (num_sms, num_threads, block_x, block_y, read_unroll):
# - Dispatch configs prioritize high SM count for scatter parallelism
# - Combine configs use lower SM count (reductions are less SM-bound)
# - Auto-tuned for DeepSeek's cluster, but customizable per deployment
#
# NVSHMEM integration: Buffer initializes NVSHMEM with IBGDA for RDMA, exchanges unique IDs across ranks,
# configures QP depth (must exceed 2 * (num_max_dispatch_tokens_per_rank + 1) for low-latency),
# and sets environment variables for traffic isolation (NVSHMEM_IB_SL), team limits, and granularity.
#
# Critical constraints:
# - NVLink required for all local ranks (check_nvlink_connections enforces this)
# - Low-latency mode requires NVSHMEM with IBGDA enabled
# - Buffer reuse: Only 2 low-latency result tensors can exist simultaneously (buffer aliasing)
# - Expert alignment: num_experts % group_size == 0 for low-latency
#
# Undefined behavior optimization: Uses aggressive PTX instructions (ld.global.nc.L1::no_allocate) for
# volatile reads on Hopper, achieving significant speedup. Disable with DISABLE_AGGRESSIVE_PTX_INSTRS=1
# if compatibility issues arise on other platforms.
# </claudes_code_comments>
