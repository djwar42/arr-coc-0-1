...
# ** Function List **
# build_instruction_prompt(instruction) - Formats instruction into DeepSeek prompt template
# build_model(model_args, training_args, checkpoint_dir) - Builds model with optional LoRA/QLoRA quantization
# train() - Main training entry point orchestrating data loading and training
# preprocess(sources, targets, tokenizer) - Tokenizes and masks instruction/response pairs
# train_tokenize_function(examples, tokenizer) - Applies preprocessing to dataset batches
# get_last_checkpoint(checkpoint_dir) - Finds most recent checkpoint for resumption
# safe_save_model_for_hf_trainer(trainer, output_dir) - Saves model state dict to disk
# _tokenize_fn(strings, tokenizer) - Core tokenization of string sequences
#
# ** Technical Review **
# This script implements supervised finetuning for DeepSeekMoE using HuggingFace Transformers
# and DeepSpeed for distributed training. Core capabilities:
# - Full precision (bf16/fp16) or quantized (4-bit/8-bit) training via BitsAndBytes
# - LoRA/QLoRA parameter-efficient finetuning with configurable rank and alpha
# - DeepSpeed ZeRO-2/ZeRO-3 for memory-efficient distributed training
# - Automatic checkpoint resumption and PEFT adapter saving
# - Flash Attention 2 support for efficient attention computation
#
# Architecture flow: load_dataset → tokenize → build_model (with optional LoRA) →
# Trainer.train → save checkpoints. Training masks instruction prefixes in labels
# (IGNORE_INDEX) so model only learns to predict responses. LoRA targets all projection
# layers (q,k,v,o,gate,down,up) with separate modules_to_save for embeddings/LM head.
#
# Key optimization: QLoRA with 4-bit NF4 quantization + LoRA enables finetuning 16B MoE
# on single A100 80GB. Full precision training requires 8xA100 40GB with ZeRO-3 offloading.
# Gradient checkpointing trades compute for memory. Auxiliary loss from MoE gating is
# automatically preserved through the finetuning process.
# </claudes_code_comments>
