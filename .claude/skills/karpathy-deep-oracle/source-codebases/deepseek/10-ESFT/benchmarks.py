...
# ** Function List **
# BaseEvaluator.infer(model, tokenizer) - runs batch inference on evaluation dataset
# BaseEvaluator.eval_metric(results) - computes task-specific metrics on predictions
# BaseEvaluator.evaluate(model, tokenizer) - orchestrates inference and metric computation
# GPT4Evaluator.query_gpt4(text) - queries GPT-4 API for LLM-as-judge evaluation
# GPT4Evaluator.parse_gpt4(response_text) - extracts numeric score from GPT-4 response
# IntentEvaluator._metrics(prediction, ground_truth) - exact match accuracy for intent classification
# SummaryEvaluator._metrics(prediction, ground_truth) - GPT-4 scored summarization quality (1-10)
# LawEvaluator._metrics(prediction, ground_truth) - GPT-4 scored legal reasoning accuracy (1-10)
# TranslationEvaluator._metrics(prediction, ground_truth) - GPT-4 scored translation quality (1-10)
#
# ** Technical Review **
# Evaluation framework for ESFT model assessment on domain-specific tasks.
#
# ARCHITECTURE:
# - BaseEvaluator: Abstract class providing inference pipeline via HF Transformers
# - GPT4Evaluator: Extends base with LLM-as-judge evaluation using GPT-4 API
# - Task-specific evaluators (Intent, Summary, Law, Translation): Implement task-specific metrics
#
# EVALUATION FLOW:
# 1. infer(): Batch inference using TextGenerationPipeline
#    - Greedy decoding (do_sample=False) for deterministic outputs
#    - max_new_tokens controls generation length
#    - return_full_text=False excludes prompt from output
# 2. eval_metric(): Computes accuracy/score per sample via _metrics()
# 3. Report average score across dataset
#
# GPT-4 EVALUATION (LLM-as-judge):
# Used for subjective tasks (summary, law, translation) where exact match inadequate
# - Queries GPT-4 with standardized prompt template (task-specific)
# - GPT-4 returns score 1-10 with reasoning
# - Regex extracts numeric score, normalizes to 0.0-1.0 range
# - 5 retry attempts with 10s backoff for API reliability
#
# TASK-SPECIFIC IMPLEMENTATIONS:
# - IntentEvaluator: Exact match accuracy (response must match ground truth exactly)
# - SummaryEvaluator: GPT-4 rates summary quality on informativeness and conciseness
# - LawEvaluator: GPT-4 judges legal reasoning accuracy and citation correctness
# - TranslationEvaluator: GPT-4 scores translation fluency and semantic accuracy
#
# Used in eval_multigpu.py for distributed model evaluation after ESFT training.
# </claudes_code_comments>
