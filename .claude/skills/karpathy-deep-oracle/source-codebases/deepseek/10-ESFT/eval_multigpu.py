...
# ** Function List **
# infer_auto_device_map(model, pp_splits, visible_devices) - creates pipeline parallel device map
# eval_model(rank, args, dataset) - evaluates ESFT model on dataset subset with adapter loaded
#
# ** Technical Review **
# Distributed evaluation script for ESFT-trained models on domain-specific benchmarks.
#
# EVALUATION PIPELINE:
# 1. Load base model (DeepSeek MoE)
# 2. Apply ESFT adapter via add_adapter() (unfreezes trainable experts, loads fine-tuned weights)
# 3. Distribute model across gpus_per_rank GPUs using pipeline parallelism (14 layers + 13 layers)
# 4. Each rank evaluates 1/world_size of dataset using task-specific evaluator from benchmarks.py
# 5. Save per-rank results to output_path.rank_N files
#
# MULTI-PROCESS PARALLELISM:
# Uses torch.multiprocessing.spawn() to parallelize evaluation across world_size processes
# Each rank handles different subset: dataset[rank::world_size] (strided slicing)
# Example: world_size=4, dataset has 100 examples
...
# Model split across gpus_per_rank=2 GPUs per rank (same as get_expert_scores.py)
# pp_splits=[14,13]: First 14 layers on GPU0, last 13 layers on GPU1
# Reduces memory usage, enables evaluation on large models
#
# EVALUATION TASKS:
# - intent: Intent classification with exact match accuracy
# - summary: Summarization quality scored by GPT-4 (1-10)
# - law: Legal reasoning accuracy scored by GPT-4 (1-10)
# - translation: Translation quality scored by GPT-4 (1-10)
#
# OUTPUT FORMAT:
# One JSONL file per rank: output_path.rank_0, output_path.rank_1, ...
# Each line: {"example": {...}, "score": float}
# Aggregate results across ranks post-evaluation for final metrics
#
# TYPICAL USAGE:
# python eval_multigpu.py \
#   --eval_dataset=translation \
#   --base_model_path=deepseek-ai/deepseek-moe-16b-base \
#   --adapter_dir=results/checkpoints/translation/best_checkpoint \
#   --output_path=results/completions/translation.jsonl \
#   --world_size=4 \
#   --gpus_per_rank=2 \
