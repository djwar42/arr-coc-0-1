"""FlashMLA Decoding Kernel Tests and Benchmarks"""

# <claudes_code_comments>
# ** Function List **
# generate_test_data(t) - Generates synthetic test data (queries, KV cache, block tables, indices)
# reference_torch(cache_seqlens, ...) - PyTorch reference implementation for correctness validation
# test_flash_mla(t) - Main test function comparing kernel output to reference
# benchmark_decoding() - Benchmarks kernel performance across different configurations
#
# ** Technical Review **
# This test suite validates FlashMLA's decoding kernels (dense and sparse) against PyTorch
# reference implementations and measures performance.
#
# Test Coverage:
# 1. Dense MLA decoding (standard KV cache attention)
# 2. Sparse MLA decoding (token-level sparse attention with topk indices)
# 3. FP8 KV cache quantization/dequantization
# 4. Variable sequence lengths within batch
# 5. Causal masking
# 6. Blocked/paged KV cache layout
#
# Test Data Generation (generate_test_data):
# - Creates batch_size sequences with optionally variable lengths (normal distribution)
# - Allocates blocked KV cache with configurable block_size (default 64)
# - Generates random block_table for paged attention simulation
# - For sparse tests: creates topk indices per query, masks unused KV entries as NaN
# - Clamps values to [-1, 1] for numerical stability
#
# Reference Implementation (reference_torch):
# - Implements standard scaled dot-product attention in PyTorch
# - Reconstructs contiguous KV from blocked layout using block_table
# - Applies causal mask (if enabled) and/or topk sparse mask
# - Computes attention weights, softmax, and output projection
# - Returns (output, lse) where lse = log-sum-exp of attention scores
#
# Test Methodology:
# - Compares kernel output vs reference using check_is_allclose (configurable tolerance)
# - For FP8 tests: quantizes/dequantizes KV cache to match kernel precision
# - Verifies both output values and log-sum-exp scores
# - Supports edge cases: zero-length sequences, all-invalid indices
#
# Benchmark Modes:
# - Memory-bound: Large batch/seqlen, tests memory throughput (target: 3000 GB/s on H800)
# - Compute-bound: High head count, tests arithmetic throughput (target: 660 TFlops dense, 410 TFlops sparse FP8)
# - Measures latency, throughput (tokens/sec), and hardware utilization
#
# Default Test Parameters:
# - h_q=128 query heads, h_kv=1 KV heads (MQA Multi-Query Attention mode)
# - d=576 QK dimension, dv=512 V dimension (MLA-specific)
# - block_size=64 (paged attention block size)
# - Batch sizes: 1-256, Sequence lengths: 1K-1M tokens
# - topk: None (dense) or 64/128/256 (sparse)
#
# Performance Targets (H800 SXM5):
# - Dense decoding: 3000 GB/s (memory-bound), 660 TFlops (compute-bound)
# - Sparse FP8 decoding: 410 TFlops
# - Variance tolerance: ±5% from targets
# </claudes_code_comments>
