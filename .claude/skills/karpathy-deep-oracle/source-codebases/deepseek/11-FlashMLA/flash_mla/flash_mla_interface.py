"""FlashMLA Python Interface - High-performance Multi-head Latent Attention kernels"""

# <claudes_code_comments>
# ** Function List **
# get_mla_metadata(cache_seqlens, ...) - Generates tile scheduler metadata for MLA decoding
# flash_mla_with_kvcache(q, k_cache, ...) - Executes MLA decoding with KV cache (sparse/dense)
# flash_mla_sparse_fwd(q, kv, indices, ...) - Sparse attention prefill kernel
# _flash_attn_varlen_forward(q, k, v, ...) - Dense attention forward pass (variable length sequences)
# _flash_attn_varlen_backward(do, q, k, v, ...) - Dense attention backward pass (variable length sequences)
# FlashAttnVarlenFunc - PyTorch autograd function for dense attention
# flash_attn_varlen_func(q, k, v, ...) - Public API for dense varlen attention
# flash_attn_varlen_qkvpacked_func(qkv, ...) - Dense varlen attention with packed QKV
# flash_attn_varlen_kvpacked_func(q, kv, ...) - Dense varlen attention with packed KV
#
# ** Technical Review **
# This module provides Python bindings for DeepSeek's FlashMLA CUDA kernels, which power
# the DeepSeek-V3/V3.2 models with optimized Multi-head Latent Attention (MLA) computation.
#
# Architecture:
# - Wraps CUDA kernels via flash_mla.cuda (PyBind11 extension)
# - Supports both sparse and dense attention patterns
# - Implements memory-efficient attention with KV caching for inference
# - Achieves 410-660 TFlops on H800, 1450 TFlops on B200
#
# Core Workflows:
# 1. MLA Decoding (Inference): get_mla_metadata() → flash_mla_with_kvcache() in loop
#    - Generates tile scheduler once, reuses across layers
#    - Supports FP8 KV cache quantization (656 bytes/token: 512B quantized + 16B scales + 128B RoPE)
#    - Token-level sparse attention via indices tensor
#
# 2. Sparse MLA Prefill: flash_mla_sparse_fwd() with selective token indices
#    - Computes attention only for topk tokens per query
#    - Returns (out, max_logits, lse) for stable computation
#    - Single-call operation, no batching dimension
#
# 3. Dense MHA Prefill/Training: flash_attn_varlen_* family
#    - Forward/backward for standard Multi-Head Attention
#    - Variable-length sequences via cumulative sequence lengths
#    - Compatible with FlashAttention API
#
# Key Design Choices:
# - Split-KV parallelism for decoding (tile_scheduler_metadata manages work distribution)
# - FP8 quantization with per-128-element scales for accuracy/efficiency balance
# - Separate sparse/dense kernels optimized for different access patterns
# - Workspace buffers for temporary storage (e.g., 32MB for fwd, dynamic for bwd)
#
# Constants:
# - SM90/SM100 architecture-specific kernel dispatch
# - MQA mode: head_dim_k=576, head_dim_v=512
# - MHA mode: head_dim_k=192/128, head_dim_v=128
# - FP8 cache format: 512 fp8_e4m3 + 4 float32 scales + 64 bfloat16 RoPE
#
# Performance Characteristics:
# - Decoding: 3000 GB/s (memory-bound), 660 TFlops (compute-bound) on H800
# - Sparse decoding (FP8): 410 TFlops (H800), 350 TFlops (B200)
# - Dense prefill: 1460 TFlops fwd, 1000 TFlops bwd (B200)
