"""FlashMLA Performance Benchmarking - Comparison vs PyTorch and FlashInfer"""

# <claudes_code_comments>
# ** Function List **
# scaled_dot_product_attention(query, key, value, ...) - Reference PyTorch SDPA implementation
# run_torch_mla(q, block_table, ...) - Benchmark PyTorch reference implementation
# run_flash_mla(q, block_table, ...) - Benchmark FlashMLA kernel
# run_flash_infer(q, block_table, ...) - Benchmark FlashInfer kernel (MLA variant)
# bench_all_methods() - Main benchmark runner comparing all implementations
#
# ** Technical Review **
# This benchmark suite compares FlashMLA decoding kernel performance against:
# 1. PyTorch reference (scaled dot-product attention)
# 2. FlashInfer (optimized paged attention library)
#
# Benchmark Methodology:
# - Uses Triton's do_bench for accurate GPU timing (warmup + multiple runs)
# - Tests identical workloads across all implementations
# - Measures latency (ms), throughput (TFlops), and memory bandwidth (GB/s)
# - Validates correctness by comparing outputs (atol/rtol checks)
#
# Test Configurations:
# - Batch sizes: 1-256
# - Sequence lengths: 1K-1M tokens
# - Query tokens per step: 1 (single token) or multiple (speculative decoding)
# - Head dimensions: h_q=128, h_kv=1 (MQA mode), d=576, dv=512
# - Causal masking: enabled/disabled
#
# Implementation Details:
#
# PyTorch Reference (run_torch_mla):
# - Naive implementation with explicit loops over batch dimension
# - Reconstructs contiguous KV from blocked layout
# - Uses torch.softmax and matrix multiply
# - Slowest but validates correctness
#
# FlashMLA (run_flash_mla):
# - Optimized CUDA kernel with split-KV parallelism
# - Paged attention via block_table
# - Tile scheduler for load balancing
# - Target: 660 TFlops (compute-bound), 3000 GB/s (memory-bound)
#
# FlashInfer (run_flash_infer):
# - Third-party optimized kernel adapted for MLA
# - Splits q/k into NoPE (no position encoding) and PE (position encoding) parts
# - Uses flashinfer.BatchDecodeWithPagedKVCacheWrapper
# - Competitive baseline for comparison
#
# Key Metrics:
# - Latency: Time per batch (ms)
# - TFlops: 2 * batch * seq_len * h_q * d^2 / time / 1e12
# - GB/s: Total memory accessed / time / 1e9
# - Speedup: FlashMLA time / baseline time
#
# Expected Results:
# - FlashMLA 5-15% faster than FlashInfer (after 2025-04-22 update)
# - 10-100× faster than PyTorch reference
# - Higher speedup for larger batch/sequence lengths (better amortization)
#
# Notes:
# - NaN markers used for unused KV cache entries (sequences shorter than max)
# - Block size typically 64 or 128 tokens
# - MLA Triton kernel reference: https://github.com/monellz/vllm (feebaa7 commit)
# </claudes_code_comments>
