---
name: vision-image-patching-oracle
description: Comprehensive knowledge base for vision-language model image patching strategies including fixed patching (ViT), adaptive patching (APT), native-resolution processing (LLaVA-UHD, Ovis), token compression, and spatial encoding. Use when questions involve image tokenization, patch size optimization, multi-resolution handling, visual token efficiency, or VLM input processing.
---

# Vision-Image-Patching-Oracle

**Your comprehensive guide to image patching strategies in Vision-Language Models**

When this skill is invoked, provide detailed, accurate information about image patching techniques, token compression strategies, and resolution handling methods in modern VLMs. Reference the knowledge base files and source research papers.

## Knowledge Base Structure

### 📐 Architecture
Core patching mechanisms and system designs:

- **[architecture/00-overview.md](architecture/00-overview.md)** - Complete patching landscape
- **[architecture/01-patch-fundamentals.md](architecture/01-patch-fundamentals.md)** - Basic patch division principles
- **[architecture/02-adaptive-patching.md](architecture/02-adaptive-patching.md)** - Dynamic patch sizing (APT, AgentViT)
- **[architecture/03-native-resolution.md](architecture/03-native-resolution.md)** - LLaVA-UHD modularization strategy
- **[architecture/04-compression-modules.md](architecture/04-compression-modules.md)** - Token compression layers
- **[architecture/05-spatial-encoding.md](architecture/05-spatial-encoding.md)** - Position embeddings & spatial schemas

### 💡 Concepts
Core principles and design philosophy:

- **[concepts/00-image-tokenization.md](concepts/00-image-tokenization.md)** - Patch-to-token transformation
- **[concepts/01-patch-size-tradeoffs.md](concepts/01-patch-size-tradeoffs.md)** - Resolution vs computation
- **[concepts/02-token-efficiency.md](concepts/02-token-efficiency.md)** - Redundancy & compression ratios
- **[concepts/03-resolution-scaling.md](concepts/03-resolution-scaling.md)** - Multi-resolution strategies

### 🔧 Techniques
Practical patching methodologies:

- **[techniques/00-fixed-patching.md](techniques/00-fixed-patching.md)** - Standard ViT approach (16×16)
- **[techniques/01-adaptive-patching.md](techniques/01-adaptive-patching.md)** - Content-aware sizing
- **[techniques/02-variable-sized-slices.md](techniques/02-variable-sized-slices.md)** - Image modularization
- **[techniques/03-compression-strategies.md](techniques/03-compression-strategies.md)** - Token reduction methods

### 🎯 Models
Implementation examples from leading VLMs:

- **[models/00-overview.md](models/00-overview.md)** - Patching approaches compared
- **[models/01-vit.md](models/01-vit.md)** - Vision Transformer baseline
- **[models/02-llava-uhd.md](models/02-llava-uhd.md)** - Variable-sized slices
- **[models/03-apt.md](models/03-apt.md)** - Adaptive Patch Transformer
- **[models/04-ovis.md](models/04-ovis.md)** - Native resolution processing
- **[models/05-deepseek-ocr.md](models/05-deepseek-ocr.md)** - 16× optical compression

### 🔍 Comparisons
Understanding approaches in context:

- **[comparisons/00-approaches-compared.md](comparisons/00-approaches-compared.md)** - Strategy overview
- **[comparisons/01-token-budgets.md](comparisons/01-token-budgets.md)** - Token counts across models
- **[comparisons/02-resolution-strategies.md](comparisons/02-resolution-strategies.md)** - Resolution handling methods

### 📋 Examples
Ready-to-use implementation patterns:

- **[examples/00-basic-patching.md](examples/00-basic-patching.md)** - Simple 16×16 division
- **[examples/01-adaptive-implementation.md](examples/01-adaptive-implementation.md)** - Dynamic patch sizing
- **[examples/02-slice-organization.md](examples/02-slice-organization.md)** - Multi-slice coordination
...
- "How does image patching work?" → `architecture/01-patch-fundamentals.md`
- "What is adaptive patching?" → `architecture/02-adaptive-patching.md`
- "How does LLaVA-UHD handle resolution?" → `architecture/03-native-resolution.md`
- "What compression techniques exist?" → `architecture/04-compression-modules.md`

**Concept Questions:**
- "Why use patches vs full images?" → `concepts/00-image-tokenization.md`
- "What's the patch size tradeoff?" → `concepts/01-patch-size-tradeoffs.md`
- "How efficient are visual tokens?" → `concepts/02-token-efficiency.md`
- "How to handle high-resolution?" → `concepts/03-resolution-scaling.md`

**Technique Questions:**
- "How does standard ViT work?" → `techniques/00-fixed-patching.md`
- "What's adaptive patching?" → `techniques/01-adaptive-patching.md`
- "How to slice images?" → `techniques/02-variable-sized-slices.md`
- "Token compression methods?" → `techniques/03-compression-strategies.md`

**Model Questions:**
- "Compare patching approaches" → `models/00-overview.md`
- "Show me ViT implementation" → `models/01-vit.md`
- "How does LLaVA-UHD work?" → `models/02-llava-uhd.md`
- "What's APT?" → `models/03-apt.md`

**Comparison Questions:**
- "Fixed vs adaptive patching?" → `comparisons/00-approaches-compared.md`
- "Token counts per model?" → `comparisons/01-token-budgets.md`
- "Resolution strategies?" → `comparisons/02-resolution-strategies.md`
...
1. **Reference source materials**: Cite papers from NotebookLM_Sources
2. **Use knowledge base**: Link to relevant .md files in this skill
3. **Be specific**: Provide formulas, code patterns, concrete examples
4. **Cross-reference**: Connect related concepts across files
5. **Stay current**: Emphasize 2024-2025 research developments

## Source Materials

Primary research from:
- **Papers**: `RESEARCH/Vision-Language Models Image Patching/NotebookLM_Sources/`
- **Key Topics**: ViT, LLaVA-UHD, Token Compression Survey, APT, Ovis, DeepSeek-OCR
- **Web Research**: Adaptive patching 2024-2025, multimodal token compression

---

## Key Innovations Covered

✅ **Fixed Patching** - Traditional ViT 16×16 approach
✅ **Adaptive Patching** - Content-aware dynamic sizing (APT)
✅ **Native Resolution** - No shape distortion (Ovis, LLaVA-UHD)
✅ **Variable Slicing** - Flexible image modularization
✅ **Token Compression** - 10-16× reduction techniques
✅ **Spatial Schemas** - Position-aware slice organization
✅ **Multi-Resolution** - Dynamic resolution support

---

## Skill Usage

This skill provides **knowledge and guidance** for:

✅ Understanding patching mechanisms
✅ Choosing optimal patch strategies
✅ Implementing compression modules
✅ Handling variable resolutions
✅ Optimizing token efficiency

❌ Not for: Automated code generation without context, deployment automation

---

**Invoke this skill** when you need authoritative information about any aspect of image patching in vision-language models.
