---
name: deepseek-ocr-oracle
description: Comprehensive knowledge base for DeepSeek-OCR vision-language model including architecture (SAM+CLIP serial design, 16x compression), training pipeline (3-stage), code implementation, and usage. Use when questions involve DeepSeek-OCR, optical compression, vision-language OCR models, SAM, CLIP, or document understanding with extreme efficiency.
---

# DeepSeek-OCR-Oracle

**Your comprehensive guide to DeepSeek-OCR: Architecture, Implementation, Training, and Usage**

When this skill is invoked, provide detailed, accurate information about DeepSeek-OCR based on the knowledge base files in this directory. Always reference actual code files and line numbers when discussing implementation details.
...
- **[architecture/00-overview.md](architecture/00-overview.md)** - Complete system architecture
- **[architecture/01-deepencoder.md](architecture/01-deepencoder.md)** - SAM + CLIP serial design (380M params)
- **[architecture/02-compression.md](architecture/02-compression.md)** - 16× spatial compression mechanism
- **[architecture/03-projector.md](architecture/03-projector.md)** - Feature fusion & MLP projection
- **[architecture/04-moe-decoder.md](architecture/04-moe-decoder.md)** - DeepSeek-3B-MoE (570M active / 3B total)
- **[architecture/05-resolution-modes.md](architecture/05-resolution-modes.md)** - Multi-resolution support (73-421 tokens)

### 💻 Code Reference
Direct code implementation guides with file:line references:

- **[code-reference/00-entry-points.md](code-reference/00-entry-points.md)** - Main execution paths
- **[code-reference/01-image-processing.md](code-reference/01-image-processing.md)** - Preprocessing & tiling
- **[code-reference/02-sam-implementation.md](code-reference/02-sam-implementation.md)** - SAM model code walkthrough
- **[code-reference/03-clip-implementation.md](code-reference/03-clip-implementation.md)** - CLIP model code walkthrough
- **[code-reference/04-projector-code.md](code-reference/04-projector-code.md)** - MlpProjector implementation
- **[code-reference/05-token-calculation.md](code-reference/05-token-calculation.md)** - Dynamic token budget logic
- **[code-reference/06-inference-flow.md](code-reference/06-inference-flow.md)** - Complete forward pass

### 🎓 Training
3-stage training pipeline documentation:

- **[training/00-overview.md](training/00-overview.md)** - Training pipeline summary
- **[training/01-stage1-deepencoder.md](training/01-stage1-deepencoder.md)** - DeepEncoder pre-training
- **[training/02-stage2-full-vlm.md](training/02-stage2-full-vlm.md)** - Full VLM training
- **[training/03-stage3-gundam.md](training/03-stage3-gundam.md)** - High-resolution fine-tuning
- **[training/04-data-engineering.md](training/04-data-engineering.md)** - Dataset construction
- **[training/05-optimization.md](training/05-optimization.md)** - Training optimizations
- **[training/06-infrastructure.md](training/06-infrastructure.md)** - HAI-LLM platform setup

### 🚀 Usage
Practical guides for using DeepSeek-OCR:

- **[usage/00-quick-start.md](usage/00-quick-start.md)** - 3-line inference example
- **[usage/01-huggingface.md](usage/01-huggingface.md)** - HuggingFace integration
- **[usage/02-resolution-selection.md](usage/02-resolution-selection.md)** - Choosing the right mode
- **[usage/03-fine-tuning.md](usage/03-fine-tuning.md)** - Custom dataset training
- **[usage/04-vllm-deployment.md](usage/04-vllm-deployment.md)** - Production deployment
- **[usage/05-gradio-demo.md](usage/05-gradio-demo.md)** - Web interface setup

### 💡 Concepts
Core ideas and design philosophy:

- **[concepts/00-optical-compression.md](concepts/00-optical-compression.md)** - The core innovation
- **[concepts/01-token-budgets.md](concepts/01-token-budgets.md)** - Why 73-421 tokens?
- **[concepts/02-forgetting.md](concepts/02-forgetting.md)** - Progressive compression over time
- **[concepts/03-tiling-strategy.md](concepts/03-tiling-strategy.md)** - Gundam mode explained
- **[concepts/04-design-philosophy.md](concepts/04-design-philosophy.md)** - Why serial architecture works

### 🔍 Comparisons
Understanding DeepSeek-OCR in context:

- **[comparisons/00-vs-standard-vlms.md](comparisons/00-vs-standard-vlms.md)** - How it differs
- **[comparisons/01-vs-arr-coc-vis.md](comparisons/01-vs-arr-coc-vis.md)** - Compression vs relevance
- **[comparisons/02-performance-metrics.md](comparisons/02-performance-metrics.md)** - Benchmarks & results

### 📁 File Index
Complete codebase reference with descriptions:

- **[file-index.md](file-index.md)** - Full file tree with summaries
...
- "How does DeepSeek-OCR work?" → `architecture/00-overview.md`
- "Explain the SAM + CLIP design" → `architecture/01-deepencoder.md`
- "How does compression work?" → `architecture/02-compression.md`
- "What are resolution modes?" → `architecture/05-resolution-modes.md`

**Code Questions:**
- "Show me the main entry point" → `code-reference/00-entry-points.md`
- "How is image preprocessing done?" → `code-reference/01-image-processing.md`
- "Where is SAM implemented?" → `code-reference/02-sam-implementation.md`
- "How are tokens calculated?" → `code-reference/05-token-calculation.md`

**Training Questions:**
- "How was it trained?" → `training/00-overview.md`
- "What data was used?" → `training/04-data-engineering.md`
- "What optimizations?" → `training/05-optimization.md`
- "Training infrastructure?" → `training/06-infrastructure.md`

**Usage Questions:**
- "How do I use it?" → `usage/00-quick-start.md`
- "Which mode should I use?" → `usage/02-resolution-selection.md`
- "Can I fine-tune?" → `usage/03-fine-tuning.md`
- "How to deploy?" → `usage/04-vllm-deployment.md`

**Concept Questions:**
- "What is optical compression?" → `concepts/00-optical-compression.md`
- "Why variable token budgets?" → `concepts/01-token-budgets.md`
- "How does forgetting work?" → `concepts/02-forgetting.md`
- "What's the design philosophy?" → `concepts/04-design-philosophy.md`
...
1. **Reference actual files**: Always cite `RESEARCH/DeepSeekOCR/DeepSeek-OCR/.../file.py:line`
2. **Use knowledge base**: Link to relevant .md files in this skill
3. **Be specific**: Provide code snippets, not just descriptions
4. **Cross-reference**: Connect related concepts across files
5. **Stay accurate**: Only state what's verified in the code

## Codebase Locations

- **Source Code**: `RESEARCH/DeepSeekOCR/DeepSeek-OCR/DeepSeek-OCR-master/DeepSeek-OCR-vllm/`
- **Documentation**: `RESEARCH/DeepSeekOCR/*.md`
- **Main Files**:
  - Entry: `run_dpsk_ocr_image.py`
  - Model: `deepseek_ocr.py`
...
✅ Learn training procedures
✅ Get usage examples
✅ Clarify concepts

❌ Not for: Automated code generation, deployment scripts, data processing

---

**Invoke this skill** when you need authoritative information about any aspect of DeepSeek-OCR.
