---
name: qwen3vl-oracle
description: Complete Qwen3-VL multimodal vision-language model documentation including Interleaved-MRoPE, DeepStack multi-layer injection, timestamp-based temporal encoding, dynamic resolution, fine-tuning pipeline, and usage guides. Use when questions involve Qwen3-VL, M-RoPE, DeepStack, timestamp alignment, video understanding, or dynamic resolution VLMs.
---

# qwen3vl-oracle Skill

**Your complete guide to Qwen3-VL architecture, training, and implementation**

## What is this?

qwen3vl-oracle provides **comprehensive documentation** on the Qwen3-VL multimodal large language model, covering its three signature innovations (Interleaved-MRoPE, DeepStack, Timestamp Alignment), training strategies, and practical implementation with direct code base references.

## Quick Start

1. **Start with INDEX.md** - Master index of all documentation
2. **Browse by category** - Architecture, Training, Codebase, Usage, Concepts, Examples
3. **Search by topic** - Interleaved-MRoPE, DeepStack, timestamp alignment
4. **Follow code references** - Every doc links to actual code with line numbers

## Organization

- **📐 Architecture (4 files)** - Core innovations and system design
- **🎓 Training (1 file)** - Fine-tuning strategies and hardware requirements
- **💻 Codebase (1 file)** - Code walkthrough with implementation details
- **📖 Usage (1 file)** - Quick start and practical guides
- **💡 Concepts (2 files)** - Deep dives into key innovations
- **🧪 Examples (1 file)** - Working code samples

**Total: 10 focused documentation files** covering the most critical aspects of Qwen3-VL

## Key Features

✅ **Modular** - One topic per file, easy navigation
✅ **Cross-referenced** - Everything linked with clear paths
✅ **Code-grounded** - Direct file references with line numbers from source codebase
✅ **Progressive** - Overview → Details → Implementation
✅ **Searchable** - Index by topic and use case
✅ **Practical** - Working examples and training scripts

## Usage Examples

```bash
# Ask about architecture innovations
"Explain Interleaved-MRoPE and how it differs from standard RoPE"
→ Reads: architecture/01-positional-encoding.md
        concepts/00-interleaved-mrope.md

# Ask about DeepStack
"How does multi-layer injection work in Qwen3-VL?"
→ Reads: architecture/02-deepstack.md

# Ask about timestamp encoding
"What are timestamp tokens and how do they improve video understanding?"
→ Reads: architecture/03-timestamp-alignment.md

# Ask about training
"How do I fine-tune Qwen3-VL with LoRA?"
→ Reads: training/00-fine-tuning-overview.md
        usage/00-quickstart.md

# Ask for implementation details
"Show me the vision_process.py code structure"
→ Reads: codebase/01-vision-process.md
        source-codebases/00-Qwen3-VL/qwen-vl-utils/src/qwen_vl_utils/vision_process.py
```

## Topics Covered

### Core Innovations
- **Interleaved-MRoPE** - Full-frequency positional encoding across time/height/width dimensions
- **DeepStack** - Multi-layer ViT feature injection (layers 6, 12, 18, 24)
- **Timestamp Alignment** - Explicit timestamp tokens for precise video temporal modeling

### Training & Deployment
- **Fine-Tuning Strategies** - LoRA vs Full training, module selection, resolution strategies
- **Hardware Requirements** - GPU configurations for different model sizes
- **DeepSpeed Integration** - ZeRO stages for distributed training

### Implementation
- **Vision Processing** - smart_resize(), fetch_image(), fetch_video() with pixel budgets
- **Dynamic Resolution** - Adaptive image sizing and patch management
- **RoPE Implementation** - Position encoding calculation in rope2d.py

### Integration
- **ARR-COC Compatibility** - How Qwen3-VL's architecture supports variable-token compression
- **HuggingFace Usage** - Complete API guide with transformers library

## Source Codebase

Complete **Qwen3-VL codebase** included at:
```
...
- 00-overview.md - Complete system architecture diagram and data flow
- 01-positional-encoding.md - Interleaved-MRoPE detailed explanation
- 02-deepstack.md - Multi-layer feature injection mechanism
- 03-timestamp-alignment.md - Video temporal encoding with timestamp tokens

**Training** - Practical fine-tuning:
- 00-fine-tuning-overview.md - Complete training guide (LoRA, full fine-tuning, hardware)

**Codebase** - Implementation details:
- 01-vision-process.md - Vision preprocessing code walkthrough

**Usage** - Getting started:
- 00-quickstart.md - Quick start guide for inference

**Concepts** - Deep dives:
- 00-interleaved-mrope.md - M-RoPE mathematical foundations
- 05-arr-coc-integration.md - Integration with ARR-COC compression

**Examples** - Working code:
- 00-basic-inference.md - Simple inference examples

## When to Use This Skill

Use qwen3vl-oracle when you need to:

- **Understand** Qwen3-VL's three core innovations (M-RoPE, DeepStack, Timestamps)
- **Implement** vision-language models with advanced positional encoding
- **Fine-tune** Qwen3-VL for custom tasks
- **Compare** Qwen3-VL's approach to other VLMs (DeepSeek-VL, Ovis, LLaVA)
- **Integrate** Qwen3-VL with relevance realization frameworks (ARR-COC)
- **Debug** position encoding, multi-layer injection, or video understanding issues

## Related Skills

- **deepseek-ocr-oracle** - DeepSeek-OCR architecture (serial SAM+CLIP design)
- **ovis-2-5-oracle** - Ovis 2.5 documentation (alternative VLM architecture)
- **john-vervaeke-oracle** - Relevance realization framework for ARR-COC integration

## Quick Navigation

**By Innovation**:
- M-RoPE → [architecture/01-positional-encoding.md](architecture/01-positional-encoding.md)
...
- Inference → [usage/00-quickstart.md](usage/00-quickstart.md)
- Code Details → [codebase/01-vision-process.md](codebase/01-vision-process.md)

**By Use Case**:
- Video understanding → [architecture/03-timestamp-alignment.md](architecture/03-timestamp-alignment.md)
- Long-context → [architecture/01-positional-encoding.md](architecture/01-positional-encoding.md)
- Fine-grained details → [architecture/02-deepstack.md](architecture/02-deepstack.md)
...
Based on Qwen3-VL by Qwen Team (Alibaba)
Documentation compiled by ARR-COC-VIS Research Team
Enriched with external sources from arXiv, LearnOpenCV, and Qwen AI blog

---

**Last Updated**: 2025-10-28
**Status**: Core documentation complete (10 files covering critical aspects)
**Coverage**: Architecture innovations, training strategies, implementation details
