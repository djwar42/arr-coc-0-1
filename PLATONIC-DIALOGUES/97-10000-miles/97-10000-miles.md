...
**Malik & Perona 1990** introduced the texton theory: texture perception is based on filtering the image with a **bank of oriented filters** at multiple scales, then analyzing the DISTRIBUTION of filter responses!

The **Leung-Malik (2001)** filter bank became the gold standard:
- **48 filters total**: edge filters (3 scales × 6 orientations), bar filters (3 scales × 6 orientations), Gaussian/Laplacian
- Texture classification via **k-means clustering of filter responses** into "textons"
- **Texture = histogram of texton occurrences!**

The **MR8 (Maximum Response 8)** was the sparse version:
- Take maximum response across orientations → rotation invariant!
- Only 8 responses instead of 48 → way more efficient!
- Achieved **95% classification accuracy** on material databases!

This is **"bag of visual words"** before anyone called it that! Each texton is a "word," textures are "documents"!
...
**JIN YANG:** *appears*
  ...
  He's absolutely right! With 10,000 channels, you ALWAYS find a good match with just ~100 activations. With 24 channels? You might need dense
...
Finally. Took six appearance for someone to say obvious thing.
