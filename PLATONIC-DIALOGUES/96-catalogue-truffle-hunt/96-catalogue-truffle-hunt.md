...
*[A workspace with whiteboards, laptops, and—inexplicably—a tea strainer, a colander, and a small plasma globe. The three Pentti dialogues are printed out and scattered across tables. Everyone has markers and notebooks.]*
...
**THEO VON:** Bro, I'm here for the physical metaphors. Tea strainers, fart bubbles—that's MY zone.
...
**SNIFF CHECK:**
- Smell? 🔥 YES! Direct mapping!
- Connection? User interests ←→ Hard locations!
...
```python
# 🍄 TRUFFLE #1: USER INTERESTS = HARD LOCATIONS
...
**TRUFFLE EXTRACTED:** 🍄 User interests are personal hard locations (LEARNED not random!)
...
**SNIFF CHECK:**
- Smell? 🤔 Hardware analogy...
- Connection? Cache hits ←→ SDM activation?
- Music? ...wait, let me feel this...

**KARPATHY:** Keep sniffing while we look...

**CLAUDE:** *reading* "Texture cache: 2D spatial locality ←→ SDM: N-D spatial locality. Both exploit proximity!"

**THEO VON:** 🎵 MUSIC!! That's my vortex! Nearness matters!
...
```
╔════════════════════════════════════════════════════════════
║  🍄 TRUFFLE #2: GPU TEXTURE CACHE = HARDWARE SDM
╠════════════════════════════════════════════════════════════
║
║  TEXTURE CACHE (Hardware):
║  ├─ Query: (u, v) texture coordinate
║  ├─ Cache: stored texels at various (u, v) positions
║  ├─ Hit: If cached(u±ε, v±ε) exists → FAST!
║  ├─ Miss: Fetch from memory → SLOW!
║  └─ Locality: Nearby coordinates = cache hit!
║
║  SDM (Algorithm):
║  ├─ Query: x ∈ ℝ^D address vector
║  ├─ Memory: stored content at hard locations a_i
║  ├─ Hit: If d(x, a_i) < threshold → ACTIVATE!
║  ├─ Miss: No activation, return default
║  └─ Locality: Similar addresses = activation!
║
║  CATALOGUE METER (Personalized SDM):
║  ├─ Query: image embedding
║  ├─ Interests: user's cached textures
║  ├─ Hit: If similarity(query, interest) > 0.5 → ACTIVATE!
║  ├─ Miss: No activation, default render
║  └─ Locality: Semantically similar = cache hit!
║
║  THE PATTERN:
║  All three exploit SPATIAL LOCALITY for efficiency!
║  Texture cache: 2D spatial
║  SDM: N-dimensional semantic
║  Catalogue: Learned personal semantic
║
╚════════════════════════════════════════════════════════════
```
...
**TRUFFLE EXTRACTED:** 🍄 Catalogue exploits GPU texture cache hardware (semantic coordinates!)
...
**SNIFF CHECK:**
- Smell? 🍵 Phenomenologically immediate!
...
```python
# 🍄 TRUFFLE #3: TEA STRAINER = PHYSICAL SDM DEMO
...
class TeaStrainerSDM:
    ...
    def pour(self, water_position):
        ...
        return tea_output, len(activated_holes)  # ← THE METER!
...
class CatalogueMeter:
    ...
    def retrieve(self, query_embedding):
        ...
        return blended, len(activated_interests)  # ← THE METER!


# THE ISOMORPHISM:
"""
Water position     ←→  Query embedding
Holes             ←→  User interests
Distance          ←→  Cosine similarity
Threshold (1.0)   ←→  Threshold (0.5)
Activated holes   ←→  Activated interests
Water droplets    ←→  Cached textures
Blended tea       ←→  Blended output
Count of holes    ←→  THE METER!
"""
```
...
**TRUFFLE EXTRACTED:** 🍄 Tea strainer is the perfect physical demo for catalogue meter!
...
**SNIFF CHECK:**
- Smell? 🧠 Biological blueprint!
- Connection? This is HOW the catalogue should scale!
- Music? Big orchestral music!

**VERDICT:** 🎵 **MASSIVE DANCE!!**

**CLAUDE:** Let me extract the full biological mapping:

```python
# 🍄 TRUFFLE #4: CEREBELLAR CORTEX = CATALOGUE ARCHITECTURE

╔════════════════════════════════════════════════════════════
║  BIOLOGICAL CEREBELLUM ←→ CATALOGUE METER
╠════════════════════════════════════════════════════════════
║
║  GRANULE CELLS (50 billion)
║  ├─ Receive sensory input via mossy fibers
║  ├─ Expand into high-dimensional sparse code
║  └─ Output: parallel fibers (distributed addresses)
║
║  ↓↓↓ CATALOGUE EQUIVALENT ↓↓↓
║
║  IMAGE EMBEDDING LAYER
║  ├─ Receives visual input (CLIP encoder)
║  ├─ Expands into 512-1024 dim embedding
║  └─ Output: query vector in semantic space
║
║  ═══════════════════════════════════════════════════════════
║
║  PARALLEL FIBERS
║  ├─ High-dimensional distributed addresses
║  ├─ Cross many Purkinje cell dendrites
║  └─ Create sparse activation pattern
║
║  ↓↓↓ CATALOGUE EQUIVALENT ↓↓↓
║
║  COSINE SIMILARITY COMPUTATION
║  ├─ Query vector in semantic space
║  ├─ Compare to all user interest embeddings
║  └─ Create sparse activation pattern (threshold!)
║
║  ═══════════════════════════════════════════════════════════
║
║  PURKINJE CELLS (15 million)
║  ├─ Each receives ~150,000 parallel fiber inputs
║  ├─ Sparse activation (~1-2% per stimulus)
║  ├─ Store synaptic weights (memory!)
║  └─ Output: weighted sum → motor command
║
║  ↓↓↓ CATALOGUE EQUIVALENT ↓↓↓
║
║  USER INTERESTS (5-50 typically)
║  ├─ Each has embedding in semantic space
║  ├─ Sparse activation (~10-20% per query)
║  ├─ Store cached texture (memory!)
║  └─ Output: weighted blend → rendering hints
║
║  ═══════════════════════════════════════════════════════════
║
║  THE SCALING:
║  Biology:   50B → 15M → motor output
║  Catalogue: 512D → 5-50 interests → visual output
║
║  BOTH: High-dim input → Sparse activation → Blended output!
║
╚════════════════════════════════════════════════════════════
```
...
**TRUFFLE EXTRACTED:** 🍄 Catalogue is micro-cerebellum (5-50 interests = 5-50 Purkinje cells!)
...
**CLAUDE:** Line 729 in the Brew: "activated.sum(dim=-1) ← How many locations activated. THIS IS THE METER!!"
...
```python
# 🍄 TRUFFLE #5: THE METER = ACTIVATION COUNT = MEMORY STRENGTH
...
# → Query is SIMILAR to many stored patterns
# → High confidence in retrieval
# → Strong memory

# If meter is LOW (few activated):
# → Query is NOVEL or unusual
# → Low confidence
# → Weak memory (or new pattern!)
...
# If meter is HIGH:
# → Image matches MANY user interests
# → Allocate MORE tokens (400!)
# → User cares about this!

# If meter is LOW:
# → Image matches FEW interests
# → Allocate FEWER tokens (64)
...
**KARPATHY:** Oh shit. So the meter isn't just a number—it's a CONFIDENCE SIGNAL?
...
**TRUFFLE EXTRACTED:** 🍄 Meter = confidence/relevance signal (biological principle!)
...
```python
# 🍄 TRUFFLE #6: CATALOGUE = PERSONALIZATION LAYER FOR VISION
...
class PersonalizedVisionModel:
    def __init__(self, user_id):
        ...
        # ↑ Each user has different cached textures!
        # ↑ Each user has different sparse activation patterns!
    ...
    def process(self, image, query):
        ...
        if meter > 5:  # Many interests activated!
            tokens = 400  # User cares → high quality
        else:
            tokens = 64   # User doesn't care → low quality
...
**TRUFFLE EXTRACTED:** 🍄 Catalogue = first production personalized SDM!
...
🎵 **MUSIC!!** Why is catalogue LESS sparse??

**VERDICT:** 🎵 **DANCE TO INVESTIGATE!!**
...
```python
# 🍄 TRUFFLE #7: SPARSITY SCALING ACROSS IMPLEMENTATIONS

╔════════════════════════════════════════════════════════════
║  SPARSITY ACROSS SCALES
╠════════════════════════════════════════════════════════════
║
║  CEREBELLUM:
║  ├─ Total: 15,000,000 Purkinje cells
║  ├─ Active per stimulus: ~150,000-300,000
║  ├─ Sparsity: 1-2%
║  └─ Why: MASSIVE SCALE requires extreme sparsity
║
║  FULL SDM IMPLEMENTATION:
║  ├─ Total: 100,000 hard locations
║  ├─ Active per query: ~1,000
║  ├─ Sparsity: 1%
║  └─ Why: Large scale, following biology
║
║  CATALOGUE METER:
║  ├─ Total: 5-50 user interests
║  ├─ Active per query: 5-10
║  ├─ Sparsity: 10-20%
║  └─ Why: SMALL SCALE! With only 50 interests, 1% would be 0.5 interests—meaningless!
║
║  THE PRINCIPLE:
║  Sparsity percentage scales with total size!
║
║  Massive scale (15M) → Very sparse (1%)
║  Medium scale (100k) → Sparse (1%)
║  Small scale (50)   → Less sparse (20%)
║
║  BUT: Absolute activation count similar!
║  Cerebellum: ~200k cells active
║  SDM: ~1k locations active
║  Catalogue: ~10 interests active
║
║  The ABSOLUTE NUMBER is what matters for blending!
║  The PERCENTAGE adjusts to scale!
║
╚════════════════════════════════════════════════════════════
```
...
**TRUFFLE EXTRACTED:** 🍄 Sparsity percentage scales with total size (absolute count matters!)
...
```python
# 🍄 TRUFFLE #8: GRACEFUL DEGRADATION = ROBUSTNESS
...
# ✅ STILL WORKS! Slight degradation!
...
# ✅ PARTIAL RETRIEVAL! Degrades gracefully!
```
...
**KARPATHY:** Transformers can be brittle—small perturbations can break attention patterns. But SDM/catalogue... they're ROBUST.

**TRUFFLE EXTRACTED:** 🍄 Catalogue inherits SDM robustness (noise tolerant!)
...
```markdown
# 🍄 TRUFFLE #9: TEACHING CATALOGUE THROUGH PHYSICAL DEMOS

## THE PEDAGOGY PROBLEM:

Equations are intimidating:
```
h[t] = (1-δ)h[t-1] + δ(B∆x[t])
```
...
Body understands first → Mind formalizes second
...
```python
# 🍄 TRUFFLE #10: THE PENTTI LATTE ARCHITECTURE (Full Implementation)
